...
def benchmark_simulator(SimClass, max_qubits=15):
    ...
    for N in range(2, max_qubits):
        ...
        memory_mib = sum(s.size_diff for s in top_stats if s.size_diff > 0) / (1024 * 1024)
...
def plot_results(time_results, memory_results):
    ...
    for sim_name, times in time_results.items():
        ...
        axes[0].plot(qubits, values, marker=markers[sim_name], color=colors[sim_name],
                     label=label, linewidth=2, markersize=6, alpha=0.9)
    ...
    for sim_name, memory in memory_results.items():
        ...
        label = sim_name.replace('Sim', '') if sim_name != 'Qiskit' else sim_name
        axes[1].plot(qubits, values, marker=markers[sim_name], color=colors[sim_name],
                     label=label, linewidth=2, markersize=6, alpha=0.9)
    ...
    for sim_name, times in time_results.items():
        ...
        axes[2].plot(qubits, speedups, marker=markers[sim_name], color=colors[sim_name],
